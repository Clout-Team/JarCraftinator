...
public class JARCraftinatorLogger implements Logger {
    ...
    public JARCraftinatorLogger(){
        ...
        if(!(logFile = new File("logs/latest.log")).exists()){
            ...
            int i = 0;
            while(new File("logs/log-" + new SimpleDateFormat("YYYY-MM-dd").format(new Date()) + "-" + i + ".log").exists()){
                i++;
            }

            File archive = new File("logs/log-" + new SimpleDateFormat("YYYY-MM-dd").format(new Date()) + "-" + i + ".log");
    ...
    public void log(String message, LogLevel level, boolean alwaysShow){
        ...
        switch(level){
            case DEBUG:
                if(JARCraftinator.getConfig().isDebug() || alwaysShow) {
